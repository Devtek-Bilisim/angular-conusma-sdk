...
export class User {
  ...
  public async changePassword(CurrentPassword: string, NewPassword: string) {
    try {
      ...
      await this.appService.changePassword(data);
      return true;
    } catch (error) {
  ...
  public async updateMeeting(meeting:MeetingModel) {
    try {
      var result = <MeetingModel>await this.appService.updateMeeting(meeting);
      return result;
    } catch (error: any) {
      throw new ConusmaException("updateMeeting", "Can't update meeting.", error);
    }
  }
  public async updateProfile() {
    try {
      await this.appService.updateProfile(this.userInfo);
    } catch (error: any) {
      throw new ConusmaException("updateProfile", "Can't update profile.", error);
    }
  }
